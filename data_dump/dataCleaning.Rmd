...
Adding function to convert column types in awardee data
```{r}
...
Standardizing dataframe format
```{r}
...
Column manipulations in awardee data
```{r}
# Rearranging Columns
...
# Applying column changes to convert chr to numeric
MVPClean <- colChange(MVPClean)
CYClean <- colChange(CYClean)
RookieClean <- colChange(RookieClean)

# Converting year to numeric as well - Cy already numeric year
MVPClean[1] <- sapply(MVPClean[1], as.numeric)
RookieClean[1] <- sapply(RookieClean[1], as.numeric)
